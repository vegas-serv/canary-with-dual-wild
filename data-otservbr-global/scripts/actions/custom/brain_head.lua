...
function brainHeadEntrance.onStepIn(creature, item, position, fromPosition)
	...
	if creature:getStorageValue(Storage.Quest.U10_80.Grimvale.BrainTimer) >= os.time() then
		creature:teleportTo(fromPosition, true)
		creature:say("You can face this boss only once in 5h.", TALKTYPE_MONSTER_SAY, false, creature)
		return true
	end
	...
	creature:setStorageValue(Storage.Quest.U10_80.Grimvale.BrainTimer, os.time() + config.waitingTime)
	creature:say("The boss has to be killed within 10 minutes after the first player has entered the room.", TALKTYPE_MONSTER_SAY, false, creature)
	return true
end
